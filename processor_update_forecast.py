...
pdf_pages = PdfPages('/content/wave_heights.pdf')

# Map station names to their respective model file
model_files = {
    '045': '/content/finalized_xgb_model.json',
    '264': '/content/finalized_xgb_model.json',
    '271': '/content/finalized_xgb_model.json',
    '179': '/content/astoria_xgb_model.json',
    '162': '/content/clatsop_xgb_model.json',
...
for stn in buoys:
    ...
    def download_wave_data_forecast(stn, start_time_f, end_time_f):
        ...
        if stn in ['045', '264', '271']:
            ...
            forecast_url = f'http://thredds.cdip.ucsd.edu/thredds/dodsC/test/WW3/46248_WW3_realtime.nc' #WW3 forecast model for Astoria Canyon
        else:
            forecast_url = f'http://thredds.cdip.ucsd.edu/thredds/dodsC/test/WW3/46243_WW3_realtime.nc' #WW3 forecast model for Clatsop Spit
    ...
    # Determine the sea state based on the current buoy SWH
    # Sea state SWH limits are based off of the NATO Standard
    def sea_state_from_swh(current_waveHs):
        sea_state_limits_swh_ft = [0, 0.33, 1.64, 4.1, 8.2, 13.1]

        sea_state_swh = 0
        for i, limit in enumerate(sea_state_limits_swh_ft):
            if current_waveHs < limit:
                sea_state_swh = i
                break

        return sea_state_swh

    pm_sea_state = pm_sea_state_from_swh(length_adjusted_height)
    sea_state = sea_state_from_swh(current_waveHs)
    ...
    output_dict = {
        "Length Adjusted PM Sea state": [pm_sea_state],
        "Buoy Sea State": [sea_state],
    ...
    plt.text(0.95, 0.01, f'Length Adjusted PM Sea state: {pm_sea_state}',
            verticalalignment='bottom', horizontalalignment='right',
            transform=plt.gca().transAxes,
            color='black', fontsize=10)
